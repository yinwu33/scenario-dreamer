...
The project was wound down on 2026-09-16. `ARCHIVE.md` is the record of what
survives, what was deleted and why, and how to rebuild anything derived; the
per-file lists and checksums are in `archive/`, generated by
`scripts/archive_manifest.py` rather than written by hand.

Three things worth knowing before using any path in this document:

- **`data/critical_scene/` no longer holds any `.ckpt`.** All 266 DDPO
  checkpoints were removed (208 GB). The v4 weights were archived into
  `data/final/` first, byte-identical and SHA256-verified; the v2/v3 weights were
  deleted outright, because this document declares them VOID and says settling v3
  needs a RETRAIN rather than a re-score. Every scene artifact, `summary.json`,
  `PROVENANCE.json` and GIF in that tree is untouched.
- **A run's `last.ckpt` is a strict superset of its `_00500.ckpt`.** The
  evaluation protocol below names `_00500.ckpt`, which no longer exists; use
  `data/final/`'s `last.ckpt`. Measured on `ppo-ppo_norm_v4kl5`, the two
...
`Trainer.fit stopped: max_steps=150000 reached`. That describes the RUN; the
archived `last.ckpt` carries `global_step=148356` (end of epoch 77), because
`ModelCheckpoint` saves per epoch. The 1.1% gap is benign, but do not quote the
file as being at step 150000.
...
both since deleted; use `data/final/`'s `last.ckpt`, see `## Archive`),
`--workers 16`.
