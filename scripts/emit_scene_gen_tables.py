...
---------------------------------
DriveSceneGen and ScenarioDreamer are carried verbatim from the templates and
flagged ``lit`` in the csv: no sample set for them is scored here, so the script
must not present them as if this pass produced them. SceneControl WAS measured
and its template placeholder is replaced.
...
LIT = {
    ...
    "ScenarioDreamer": {
        "agent": {"nearest_dist_jsd": 0.07, "lat_dev_jsd": 0.07, "ang_dev_jsd": 0.07,
                  "length_jsd": 0.40, "width_jsd": 0.25, "speed_jsd": 0.36,
                  "collision_rate": 5.4},
        "lane": {"route_length_mean": 38.23, "route_length_std": 12.78,
                 "endpoint_dist_mean": 0.32, "endpoint_dist_std": 0.90,
                 "frechet_connectivity": 0.17, "frechet_density": 1.05,
                 "frechet_reach": 0.56, "frechet_convenience": 3.81},
    },
}
PARAMS = {"DriveSceneGen": "-", "SceneControl": "-", "ScenarioDreamer": "$376M$",
          "AdvScene-Base": "$236M$", "AdvScene-RL": "$236M$"}
...
ROWS = [
    ...
    ("ScenarioDreamer", r"ScenarioDreamer~\cite{rowe2025scenario} [TODO]", r"$\times$",
     "lit"),
    # Base is the UNCONDITIONAL cache, by choice. The RL caches are generated
    # under cond_adv_ego, so the two differ in conditioning as well as in the RL
    # stage and the goal columns are not a matched before/after: measured against
    # base/init_scene (same conditioning, scored and kept in the metrics json)
    # the RL rows are flat, 0.98-1.03x on eight of eleven columns. The caption
    # says so rather than leaving the gap to be read as an RL effect.
...
HEADER_NOTE = [
    ...
    r"% do not hand-edit: rerun the script instead. DriveSceneGen and",
    r"% ScenarioDreamer are carried from the template, not scored by this pass.",
]
...
def render_agent(table: dict) -> str:
    ...
    return "\n".join([
        *HEADER_NOTE,
        ...
        r"adversary included. AdvScene-RL pools the twelve adversary configurations "
        r"and its $\pm$ is the spread across those models, which is neither a "
        r"sampling error nor a seed variance. AdvScene-Base is sampled "
        r"unconditionally while AdvScene-RL is sampled at the conditioning its "
        r"fine-tuning uses, so the goal columns differ by that conditioning as "
        r"well as by the RL stage; against a conditioning-matched base the RL "
        r"rows move by $0.98$--$1.03\times$ on eight of the eleven columns.}",
...
def render_lane(table: dict) -> str:
    ...
    return "\n".join([
        *HEADER_NOTE,
        ...
        r"a function of that graph. AdvScene-Base and AdvScene-RL share one "
        r"stage-1 lane sample, since the RL stage trains the adversary branch "
        r"only.}",
