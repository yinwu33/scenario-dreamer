#!/bin/bash
# table_main_v7: hierarchical_v7 DDPO at 500 iterations, scored in the
# ADVERSARIAL scope (ego vs the generated adversary) to match the reward.
#
# v7 keeps v6's uncredited ram but carves out the case where the ego HAD been
# closing on the adversary inside its fault cone before the contact landed
# elsewhere: those keep the TTC band instead of scoring 0. Offline on v6's own
# artifacts that rescores 12 of 7871 scenes (0.15%), 9 of them in ppo-idm, so
# most cells here will reproduce v6 up to seed noise.
#
# That makes this table two things at once. For ppo-idm it tests the carve-out.
# For the other eleven it is an independent repeat of v6, which is worth having:
# P(minTTC_ego-adv < tau) turned out to be the one metric with the events to
# resolve anything (283 -> 463 for v4, 283 -> 379 for v6 over the 12 cells,
# paired McNemar p = 7e-15 and 5e-06), and a second run says whether that
# reproduces.
#
...
OLD=data/critical_scene/table_main_v6
OUT=data/critical_scene/table_main_v7
mkdir -p $OUT
...
for spec in $CELLS; do
  ...
  RUN=${CELL}_v7
  CKDIR=data/critical_scene/critical_scene_ddpo_ldm_adv_ddim_${RUN}_hier_v7
  CKPT=$CKDIR/critical_scene_ddpo_ldm_adv_ddim_${RUN}_hier_v7_00$(printf %03d $ITERS).ckpt
  CDIR=$OUT/$CELL
  ...
  if [ -f "$CKPT" ]; then
    ...
    .venv/bin/python train.py --config-name config_ldm_adv_ddpo \
      ...
      hydra.run.dir=$PROJECT_ROOT/slurm_logs/tmv7_$CELL
